...
async def get_navigation_items(db: Session = Depends(get_db)):
    ...
    items = db.query(models.NavigationItem).all()
    return items
...
async def create_navigation_item(
    ...
    existing_item = db.query(models.NavigationItem).filter(models.NavigationItem.id == item.id).first()
    if existing_item:
        raise HTTPException(
...
async def update_navigation_item(
    ...
    # Find the item
    db_item = db.query(models.NavigationItem).filter(models.NavigationItem.id == item_id).first()
    if not db_item:
        raise HTTPException(
...
async def get_user_permissions(
    ...
    permissions = db.query(models.UserNavigationPermission).filter(
        models.UserNavigationPermission.user_id == user_id
    ).all()
...
async def save_user_permissions(
    ...
    db.query(models.UserNavigationPermission).filter(
        models.UserNavigationPermission.user_id == user_id
    ).delete()
    ...
    for nav_item_id, is_enabled in permissions.navItems.items():
        ...
        nav_item = db.query(models.NavigationItem).filter(models.NavigationItem.id == nav_item_id).first()
        if not nav_item:
            continue  # Skip non-existent items
        
        # Create permission
        db_permission = models.UserNavigationPermission(
            user_id=user_id,
